...
未実装の改善項目は[クライアントテスト基盤のTODO](todo.md#クライアントテスト基盤)にまとめている
...
- Minecraftの接続では汎用ランナーのスイートを差し替え、外部入力モードでキーとマウスの状態を差し替える計装を無効にする
  - ワールドとテスト用アイテムの準備はJava側に残し、ホバーとCtrl+Cは外部ドライバーから送る
  - `1.21.1-fabric`でワールド設定、原木のコピーとアイテム不変、空スロットでコピーしないことを確認した
  - コピー内容はMinecraftとxclipの両方から照合し、キーが解放されたこともGLFWから確認する
- tickの間隔だけで描画への反映完了を判断しない
  - 入力送信直後にホバースロットが未更新となる場合があるため、実際のホバースロットを期限付きで待つ
  - 対象スロットの状態を書き換えて成功扱いにしない
- クリップボードの厳密な比較ではシェルのコマンド置換による末尾改行の除去を避ける
  - 期待値と取得値をファイルへ保存し、バイト単位で照合する
  - 操作と応答のJSONLは1行に1オブジェクトを保存する
