...
- コピーキーをMinecraftの操作設定にある`ItemNameCopy`カテゴリから変更可能
- 既定の`C`を含め、長押しでは一度だけコピー
...
空でないスロットへカーソルを合わせ、コピーキーを押す
既定は`C`で、Minecraftの「設定」→「操作設定」→「キー割り当て」にある`ItemNameCopy`カテゴリから変更できる
空のスロットや対象外の画面ではクリップボードを変更せず、成功通知も表示しない

コピーするのは表示名のプレーンテキストだけで、装飾、アイテムID、個数、Lore、NBT、Data Componentは含めない
名前の前後の空白や改行はそのまま残す

ワールド画面で手に持っているアイテム、レシピビューアー独自の疑似スロット、コピー形式用の独自設定画面には対応していない
